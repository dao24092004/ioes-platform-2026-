...
const InstructorLayout: React.FC<InstructorLayoutProps> = ({ title, subtitle, children, headerActions }) => {
  ...
  const sections: NavSection[] = [
    {
      title: t('instructor.nav.overview'),
      items: [
        { path: '/instructor', label: t('instructor.nav.dashboard'), icon: <DashboardIcon /> },
        { path: '/instructor/courses', label: t('instructor.nav.myCourses'), icon: <BookIcon /> },
        { path: '/instructor/exams', label: t('instructor.nav.exams'), icon: <ExamIcon />, badge: '3', badgeVariant: 'warning' },
        { path: '/instructor/students', label: t('instructor.nav.students'), icon: <UsersIcon /> },
      ],
    },
    {
      title: t('instructor.nav.community'),
      items: [
        { path: '/instructor/discussions', label: t('instructor.nav.discussions'), icon: <ChatIcon /> },
        { path: '/instructor/messages', label: t('instructor.nav.messages'), icon: <MessageIcon />, badge: '5' },
      ],
    },
    {
      title: t('instructor.nav.create'),
      items: [
        { path: '/instructor/courses/create', label: t('instructor.nav.createCourse'), icon: <PlusCircleIcon /> },
        { path: '/instructor/exams/create', label: t('instructor.nav.createExam'), icon: <DocumentIcon /> },
      ],
    },
    {
      title: t('instructor.nav.analytics'),
      items: [
        { path: '/instructor/analytics', label: t('instructor.nav.stats'), icon: <ChartIcon /> },
        { path: '/instructor/reports', label: t('instructor.nav.reports'), icon: <ReportIcon /> },
        { path: '/instructor/grading', label: t('instructor.nav.grading'), icon: <GradingIcon />, badge: '12' },
      ],
    },
    {
      title: t('instructor.nav.aiTools'),
      items: [
        { path: '/instructor/ai-question', label: t('instructor.nav.aiQuestion'), icon: <SparklesIcon /> },
        { path: '/instructor/ai-assistant', label: t('instructor.nav.aiAssistant'), icon: <ChatBubbleIcon /> },
        { path: '/instructor/copyright', label: t('instructor.nav.copyright'), icon: <CopyrightIcon /> },
      ],
    },
    {
      title: t('instructor.nav.account'),
      items: [
        { path: '/instructor/profile', label: t('instructor.nav.profile'), icon: <UserIcon /> },
        { path: '/instructor/settings', label: t('instructor.nav.settings'), icon: <SettingsIcon /> },
      ],
    },
  ];
  ...
  const initials = (user?.full_name || 'GV')
    .split(' ')
    .map(s => s.charAt(0))
    .slice(0, 2)
    .join('')
      ...
      >
                      ...
                      {item.badge && (
                        <span
                          className={`px-2 py-0.5 text-[10px] font-bold text-white rounded-full ${
                            item.badgeVariant === 'warning' ? 'bg-amber-500' : 'bg-blue-600'
                          }`}
                        >
                          {item.badge}
                        </span>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-semibold text-sm bg-gradient-to-br from-amber-500 to-orange-500">
              {initials}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold truncate">{user?.full_name || 'Instructor'}</div>
              <div className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                {t('sidebar.role.instructor')}
              </div>
            </div>
            <button
...
const CopyrightIcon = () => (
  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <circle cx="12" cy="12" r="10" />
    <path d="M15 9.5a3.5 3.5 0 00-3-1.5c-2 0-3.5 1.5-3.5 4s1.5 4 3.5 4a3.5 3.5 0 003-1.5" />
  </svg>
);
