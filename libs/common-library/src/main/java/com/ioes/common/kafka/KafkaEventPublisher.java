 ...
 * <p>
 * Wraps a Spring {@link KafkaTemplate} and serializes the envelope as JSON.
 * <p>
 * Topic routing strategy:
 * <ul>
 *   <li>{@code publish(event, source)} uses the lowercase eventType as the
 *       topic name (e.g. "UserRegistered" -> "userregistered"). For
 *       production, prefer the explicit topic overload below.</li>
 *   <li>{@code publishEnvelope(topic, envelope)} sends to the exact topic
