 ...
 * itself.
 *
 * <p>Connection details are read from the same environment variables as
 * {@code application.yml}'s {@code spring.datasource.*} block, rather than
 * being hardcoded here, per {@code PROJECT_RULES.md} section I.3 — no
 * default fallback values are kept for any of them. {@code POSTGRES_HOST},
 * {@code POSTGRES_PORT}, {@code NOTIFICATION_DB_NAME}, {@code POSTGRES_USER}
 * and {@code POSTGRES_PASSWORD} must all be set to point this test at an
 * instance; if any is missing, the class is skipped the same way it is
...
class NotificationJpaRepositoryTest {
    ...
    static void assumeDatabaseIsReachable() {
        Assumptions.assumeTrue(JDBC_URL != null && JDBC_USERNAME != null && JDBC_PASSWORD != null,
                "Skipping NotificationJpaRepositoryTest: one or more of POSTGRES_HOST, POSTGRES_PORT, "
                        + "NOTIFICATION_DB_NAME, POSTGRES_USER, POSTGRES_PASSWORD is not set. This is a "
                        + "dev/CI-optional live-DB test; set these env vars (see .env.example) and run the "
                        + "local ioes-postgres container to exercise it.");
