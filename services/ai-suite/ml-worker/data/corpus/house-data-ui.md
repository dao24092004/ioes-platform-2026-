...
doc_id: house-data-ui
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/scripting/house-data-ui
...
- `streetSelect`: The "choose-street" `` element.
- `bedroomSelect`: The "choose-bedrooms" `` element.
- `bathroomSelect`: The "choose-bathrooms" `` element.
- `form`: The overall `` element that contains the `` elements.
- `resultCount`: The "result-count" `` element, which updates to display the number of results returned after each search.
- `output`: The "output" `` element, which displays the search results.
...
- `initializeForm()`: This will query the data and populate the `` elements with the possible values that could be searched for.
- `renderHouses()`: This will filter the data based on the `` element values, and render the results.
...
Now you need to write the contents of the `initializeForm()` function. This will query the data stored inside `houses` and use it to populate the `` elements with `` elements representing all of the different values that could be filtered for. At the moment, the `` elements only contain a single `` element with a value of `""` (an empty string), which represents all values. The user can choose this option if they don't want the results to be filtered by that field.

Inside the function body, write code that does the following:

1. Create `` elements for all the different street names inside the "choose-street" ``. There are a few ways you could do this, but we'd recommend creating a temporary array then looping through all the objects inside `houses`. Inside the loop, check whether your temporary array includes the `street` property of the current house. If not, add it to the temporary array and add an `` to the "choose-street" `` that includes the `street` property as its value.
2. Create options for all the possible bedroom number values inside the "choose-bedrooms" ``. To do this, you could loop through the `houses` array and determine what the largest `bedrooms` value is, then write a second loop that adds an `` to the "choose-bedrooms" `` for each number from `1` to that largest value.
3. Create options for all the possible bathroom number values inside the "choose-bathrooms" ``. This can be solved using the same technique as the previous step.

> [!NOTE]
> You _could_ just hardcode the `` elements inside the HTML, but that would only work for this exact data set. We want you to write JavaScript that will correctly populate the form regardless of the data values provided (each house object would have to have the same structure).
...
Next, you need to complete the `renderHouses()` function body. This will filter the data based on the `` element values, and render the results to the UI.
...
1. First, you need to filter the data. This is probably best achieved using the array `filter()` method, which returns a new array containing only the array elements that match the filter criteria.
   1. This is a fairly complex `filter()` function to write. You need to test whether the `street` property of the house is equal to the selected value of the "choose-street" ``, and whether the `bedrooms` property of the house is equal to the selected value of the "choose-bedrooms" ``, and whether the `bathrooms` property of the house is equal to the selected value of the "choose-bathrooms" ``.
   2. Each component of the test always needs to return `true` if the associated `` value is `""` (the empty string, which represents all values). You can achieve this by "short-circuiting" each check.
...
2. Output the number of filtered search results into the "result-count" `` element, using the string structure "Results returned: number".
3. Empty the "output" `` element, so it doesn't have any child HTML elements. If you don't do this, each time a search is performed the results will be added to the end of the previous results rather than replacing them.
4. Create a new function inside `renderHouses()` called `renderHouse()`. This function needs to take a house object as an argument, and do two things:
   ...
   2. Add an `` element inside the "output" `` element containing the house's number, street name, bedroom and bathroom count, total room area, and price. you can vary the structure if you like, we'd like it to be similar to this HTML snippet:
