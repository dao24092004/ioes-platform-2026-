...
doc_id: useful-string-methods
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/scripting/useful-string-methods
...
Now that we've looked at the very basics of strings, let's move up a gear and start thinking about what useful operations we can do on strings with built-in methods, such as finding the length of a text string, joining and splitting strings, substituting one character in a string for another, and more.
      ...
      Prerequisites:
      An understanding of HTML and the fundamentals of CSS. Knowledge of string basics.
...
although the variable itself isn't an object, it still has a large number of properties and methods available to it, by virtue of being usable as an object when accessing properties. You can see this if you go to the String object page and look down the list on the side of the page!
...
This is easy — you use the String.prototype.length", "length property. Try entering the following lines:
...
Sometimes you'll want to find if a smaller string is present inside a larger one (we generally say _if a substring is present inside a string_). This can be done using the  method, which takes a single parameter — the substring you want to search for.

It returns `true` if the string contains the substring, and `false` otherwise.

Often you'll want to know if a string starts or ends with a particular substring. This is a common enough need that there are two special methods for this:  and :

## Finding the position of a substring in a string

You can find the position of a substring inside a larger string using the  method. This method takes two parameter", "parameters – the substring that you want to search for, and an optional parameter that specifies the starting point of the search.
...
You can extract a substring from a string using the  method. You pass it:
...
> [!NOTE]
> `slice()` has other options too; study the  page to see what else you can find out.

## Changing case

The string methods  and  take a string and convert all the characters to lower- or uppercase, respectively. This can be useful for example if you want to normalize all user-entered data before storing it in a database.

Let's try entering the following lines to see what happens:

## Updating parts of a string

You can replace one substring inside a string with another substring using the  method.
...
Be aware that `replace()` in this form only changes the first occurrence of the substring. If you want to change all occurrences, you can use :
