...
doc_id: responsive-design
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/css-layout/responsive-design
...
> [!NOTE]
> Scrimba has a tutorial called [Aside: Media queries](https://scrimba.com/frontend-path-c0j/~0j3?via=mdn) [_MDN learning partner_](/en-US/docs/MDN/Writing_guidelines/Learning_content#partner_links_and_embeds), which provides an interactive introduction to media queries plus a challenge to test that you understand the basics.
...
There is a solution, and it involves using . If you add the `vw` unit to a value set using a fixed size such as `em`s or `rem`s then the text will still be zoomable. Essentially, the `vw` unit adds on top of that zoomed value:
...
If you look at the HTML source of a responsive page, you will usually see the following meta tag in the `` of the document.
...
There are a number of other options you can put inside the `content` attribute of the viewport meta tag — see the [``](/en-US/docs/Web/HTML/Reference/Elements/meta/name/viewport) reference for more details.
...
- [The Frontend Developer Career Path](https://scrimba.com/the-frontend-developer-career-path-c0j?via=mdn) [_MDN learning partner_](/en-US/docs/MDN/Writing_guidelines/Learning_content#partner_links_and_embeds) from Scrimba teaches all you need to know to be a competent front-end web developer, with fun interactive lessons and challenges, knowledgeable teachers, and a supportive community. Go from zero to landing your first front-end job! Many of the course components are available as standalone free versions. This includes a module on responsive design.
