...
doc_id: math
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/scripting/math
license: CC BY-SA 2.5 (MDN Web Docs)
---

At this point in the course, we discuss math in JavaScript — how we can use Operator","operators and other features to successfully manipulate numbers to do our bidding.
...
The second bit of good news is that, unlike some other programming languages, JavaScript has only one data type to represent basic numbers — both integers and decimals. You guessed it, Number. This means that whatever type of numbers you are dealing with in JavaScript, you handle them in the same way.

> [!NOTE]
> JavaScript has a second number type, BigInt, used for very, very large integers. But in this course, we'll only worry about `Number` values.
...
3. Now let's check that both our original variables are of the same data type. There is an operator called Operators/typeof", "typeof in JavaScript that does this. Enter the two lines below as shown:
...
> [!NOTE]
> You'll sometimes see numbers involved in arithmetic referred to as Operand", "operands.

> [!NOTE]
> You may sometimes see exponents expressed using the older  method, which works in a very similar way. For example, in `Math.pow(7, 3)`, `7` is the base and `3` is the exponent, so the result of the expression is `343`. `Math.pow(7, 3)` is equivalent to `7**3`.
...
In this exercise, you will manipulate some numbers and operators to change the size of a box. The box is drawn using a browser API called the Canvas API", "", "", "true. There is no need to worry about how this works — just concentrate on the math for now. The width and height of the box (in pixels) are defined by the variables `x` and `y`, which are initially both given a value of 50.
