...
doc_id: events
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/scripting/events
...
As we saw in the last example, objects that can fire events have an  method, and this is the recommended mechanism for adding event listeners.
...
- a function to call when the event happens. In our case, the defined anonymous function generates a random RGB color and sets the background-color of the page [``](/en-US/docs/Web/HTML/Reference/Elements/body) to that color.
...
Some events, such as `click`, are available on nearly any element. Others are more specific and only useful in certain situations: for example, the [`play`](/en-US/docs/Web/API/HTMLMediaElement/play_event) event is only available on elements that have play functionality, such as video.
...
By making more than one call to , providing different handlers, you can have multiple handler functions running in response to a single event:
...
The event model is similar to the web events model, but a bit different — event listener properties are written in camel_case", "camel case (such as `onMessage` rather than `onmessage`), and need to be combined with the `addListener` function.
