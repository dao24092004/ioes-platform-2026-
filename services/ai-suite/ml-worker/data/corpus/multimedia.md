...
doc_id: multimedia
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/accessibility/multimedia
...
Another category of content that can create accessibility problems is multimedia. Video, audio, and image content need to be given proper textual alternatives so that they can be understood by assistive technologies and their users. This article shows how.
      ...
      Learning outcomes:
          ...
          The issues with native media players, and how to create your custom ones.
          The purpose of audio transcripts and text tracks (captions, subtitles, etc.) in making audio and video content accessible.
...
- Link to the .vtt file with the track element. `` should be placed within `` or ``, but after all `` elements. Use the [`kind`](/en-US/docs/Web/HTML/Reference/Elements/track#kind) attribute to specify whether the cues are subtitles, captions, or descriptions. Furthermore, use [`srclang`](/en-US/docs/Web/HTML/Reference/Elements/track#srclang) to tell the browser what language you have written the subtitles in.
...
> [!NOTE]
> Text tracks and transcriptions also help you with SEO, since search engines especially thrive on text. Text tracks even allow search engines to link directly to a spot partway through the video.
