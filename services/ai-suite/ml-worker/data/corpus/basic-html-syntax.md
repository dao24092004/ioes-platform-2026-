...
doc_id: basic-html-syntax
source_url: https://developer.mozilla.org/en-US/docs/learn-web-development/core/structuring-content/basic-html-syntax
...
In this article, we cover the fundamentals of HTML, including terminology, syntax, and structure. Along the way, you'll complete some interactive challenges to become familiar with writing basic HTML.
      ...
      Prerequisites:
        ...
        Basic software installed, and basic knowledge of working with files.
...
HTML (HyperText Markup Language) is a _markup language_ that tells web browsers how to structure the web pages you visit. HTML consists of a series of Element", "elements, which you use to enclose, wrap, or _mark up_ different parts of content to make it appear or act in a certain way. The enclosing Tag", "tags can make content into a hyperlink to link to another page, italicize words, and so on. For example, consider the following line of text:

We could specify that this text is a paragraph by enclosing it in paragraph (p) tags:

Or, we could specify that this text is a top-level heading by enclosing it in [``](/en-US/docs/Web/HTML/Reference/Elements/Heading_Elements) tags:
...
> [!NOTE]
> Tags in HTML are not case-sensitive. This means they can be written in uppercase or lowercase. For example, a title tag could be written as ``, ``, ``, ``, etc., and it will work. However, it is best practice to write all tags in lowercase for consistency and readability.

## Anatomy of an HTML element

Let's further explore our paragraph element from the previous section:

![A sample code snippet demonstrating the structure of an html element. My cat is very grumpy .](grumpy-cat-small.png)
...
> [!NOTE]
> Head on over to our learning partner Scrimba's [HTML tags](https://scrimba.com/learn-html-and-css-c0p/~02?via=mdn) [_MDN learning partner_](/en-US/docs/MDN/Writing_guidelines/Learning_content#partner_links_and_embeds) scrim for an interactive explanation of HTML tags.
...
2. Wrap the line of text with the tags `` and ``. To _open the element_, put the opening tag (``) at the start of the line. To _close the element_, put the closing tag (``) at the end of the line. Doing this should format the rendered text as italic text.
...
There is a right and wrong way to do nesting. In the code block above, we open the `` element first, then open the `` element. For proper nesting, we close the `` element first, then close the ``.
...
Not all elements follow the pattern of an opening tag, content, then a closing tag. Some elements consist of a single tag, which is typically used to insert/embed something in the document. Such elements are called void element", "void elements, which means "elements that cannot contain other HTML content".
...
> [!NOTE]
> In some HTML examples, you'll see a `/` added at the end of a void element's tag, for example ``. This is a different style of markup syntax, which isn't wrong, but this "closing slash" is not needed.
...
Now it's your turn again. In this section you are going to explore the img element, which is used to display an image on the page. The `` element can take several attributes, including:
...
3. Back in the MDN Playground, add the `src` attribute to the `` element and set its value to the URL from Step 2.
...
For example, consider the [`disabled`](/en-US/docs/Web/HTML/Reference/Elements/input#disabled) attribute, which you can assign to form input elements to stop the user entering data into them. For example:

As shorthand, it is acceptable to write the `disabled` attribute without a value:

For reference, let's also provide a non-disabled `` element so you can compare and contrast (note how the `disabled` inputs are somewhat greyed out in the below rendering):

The above HTML snippets render like so:

> [!NOTE]
> The label elements included in the above code provide a way to associate descriptive labels with form elements. We included them because it is a best practice, and to provide some separation between the form inputs.
...
1. Just below the opening tag of the body element, add a main title for the document. This should be wrapped by `` opening and closing tags.
...
You do this with character reference", "character references. These are special codes that represent characters, to be used in these exact circumstances. Each character reference starts with an ampersand (&), and ends with a semicolon (;).
...
You can see that the first paragraph has gone wrong, because the browser has interpreted the second instance of `` as starting a new paragraph. The second paragraph renders fine because the angle brackets of the content "&lt;p&gt;" are represented by character references.
